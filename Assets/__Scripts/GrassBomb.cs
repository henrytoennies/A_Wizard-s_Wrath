...
public class GrassBomb : MonoBehaviour
{
    ...
    void Explode()
    {
        ...
        p.rigid.velocity = Vector3.left * def.velocity * 0.5f;
        p = MakeProjectile();
        p.transform.rotation = Quaternion.Euler(0, 0, 135);
        p.rigid.velocity = Quaternion.Euler(0,0,45) * Vector3.left * def.velocity * 0.5f;
        p = MakeProjectile();
        p.transform.rotation = Quaternion.Euler(0, 0, 180);
        p.rigid.velocity = Vector3.up * def.velocity * 0.5f;
        p = MakeProjectile();
        p.transform.rotation = Quaternion.Euler(0, 0, 225);
        p.rigid.velocity = Quaternion.Euler(0,0,45) * Vector3.up * def.velocity * 0.5f;
        p = MakeProjectile();
        p.transform.rotation = Quaternion.Euler(0, 0, 270);
        p.rigid.velocity = Vector3.right * def.velocity * 0.5f;
        p = MakeProjectile();
        p.transform.rotation = Quaternion.Euler(0, 0, 315);
        p.rigid.velocity = Quaternion.Euler(0,0,45) * Vector3.right * def.velocity * 0.5f;
        p = MakeProjectile();
        p.transform.rotation = Quaternion.Euler(0, 0, 0);
        p.rigid.velocity = Vector3.down * def.velocity * 0.5f;
        p = MakeProjectile();
        p.transform.rotation = Quaternion.Euler(0, 0, 45);
        p.rigid.velocity = Quaternion.Euler(0,0,45) * Vector3.down * def.velocity * 0.5f;
